...
public class DialogueManager : MonoBehaviour
{
    ...
    */
    ...
    // Edited to work with Player_Move_Update
    private PlayerMovement playerMovement;

    /* ^ IMPORTANT IMPORTANT IMPORTANT IMPORTANT ^*/

    [Tooltip("Optional reference to PlayerShooter to disable when speaking.")]
    ...
    {
        // This is not a great way of freezing the player, but is done so that PlayerMovement doesn't have to change
        if (GameObject.FindGameObjectWithTag("Player").TryGetComponent<PlayerMovement>(out PlayerMovement player_move))
        {
            playerMovement = player_move;
            playerMovement = GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerMovement>();
            playerMovement.gameObject.GetComponent<Rigidbody2D>().velocity = Vector2.zero;
            playerMovement.gameObject.GetComponent<Animator>().SetBool("IsRunning", false);
